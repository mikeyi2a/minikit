...
export function DualSlider({
  ...
  const [activeHandle, setActiveHandle] = React.useState<Handle | null>(null);
  const [liveRange, setLiveRange] = React.useState<{
    start: number;
    end: number;
  } | null>(null);
  const draggingRef = React.useRef(false);
  const activeHandleRef = React.useRef<Handle | null>(null);
  const trackRef = React.useRef<HTMLDivElement>(null);

  const startPct = liveRange
    ? liveRange.start
    : ((start - min) / (max - min)) * 100;
  const endPct = liveRange
    ? liveRange.end
    : ((end - min) / (max - min)) * 100;
        ...
        >
          <div
            ...
            style={{
              ...
              background:
                "color-mix(in srgb, var(--mk-text) 80%, transparent)",
            }}
          />
          <div
            role="slider"
