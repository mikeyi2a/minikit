...
The `tailwind.css` import registers the package's compiled output with your Tailwind build — no path math, no editor config.
...
/* (path is relative to this CSS file; adjust the `..` count if your
   app is deeper than one level from node_modules) */
```
